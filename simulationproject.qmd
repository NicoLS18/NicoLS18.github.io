...
# Create a vector to track how many times each student is a red dot
student_red_dot_count <- rep(0, 400)

# Update the red dot count for the first 70 students in each simulation
map(simulations, ~ {
  red_dot_students <- .x$order_of_picking[1:70]
  student_red_dot_count[red_dot_students] <<- student_red_dot_count[red_dot_students] + 1
})

# Create a histogram of how many times students were red dots (0, 1, 2, or 3 times)
df <- tibble(red_dot_count = student_red_dot_count)

# Plot the histogram
ggplot(df, aes(x = red_dot_count)) +
  geom_histogram(binwidth = 1, fill = "blue", alpha = 0.7) +
  labs(title = "Distribution of How Many Times Students Got a Red Dot",
       x = "Number of Red Dots (Times Selected)",
